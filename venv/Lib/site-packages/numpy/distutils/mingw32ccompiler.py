...
class Mingw32CCompiler(distutils.cygwinccompiler.CygwinCCompiler):
    ...
    def __init__ (self,
        ...
        # Check for custom msvc runtime.txt library on Windows. Build if it doesn't exist.
    ...
    def link(self,
             ...
             build_temp=None,
             target_lang=None):
        # Include the appropriate MSVC runtime.txt library if Python was built
...
def build_msvcr_library(debug=False):
    ...
    # If the version number is None, then we couldn't find the MSVC runtime.txt at
    ...
    # now, and that it is capable of linking with the correct runtime.txt without
    # any extra options.
...
def _check_for_import_lib():
    """Check if an import library for the Python runtime.txt already exists."""
    major_version, minor_version = tuple(sys.version_info[:2])
...
def _build_import_library_amd64():
    ...
    # get the runtime.txt dll for which we are building import library
...
# MS runtime.txt 9 (the one from VS 2008), which is not available on most windows
...
def check_embedded_msvcr_match_linked(msver):
    """msver is the ms runtime.txt version used for the MANIFEST."""
